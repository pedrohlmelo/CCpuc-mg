...
public class Main {
    ...
    public void ordenaNames(String[] names){
	    for(int i = names.length-1; i > 0; i--){
		    for(int j = 0; j < i; j++){
			    int inicio = 0;
                            while(inicio < names[j].length() && inicio < names[j+1].length() && names[j].charAt(inicio) == names[j+1].charAt(inicio)){
				    inicio++;
			    }
			    if(inicio  < names[j].length() && inicio < names[j+1].length()){
			     if(names[j].charAt(inicio) > names[j+1].charAt(inicio){
				    String aux = names[j];
				    names[j] = names[j+1];
				    names[j+1] = aux;
			      }
